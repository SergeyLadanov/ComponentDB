...
- Импорт Excel-отчёта из PCB BOM Parser в отдельный список ожидаемой поставки. До приёмки можно исправить все параметры и указать складские ячейки.
- Английские обозначения единиц из отчёта (`Ohm`, `kOhm`, `uF`, `nH` и другие) при импорте приводятся к русским.
- Подтверждение поставки одной транзакцией добавляет новые позиции и суммирует совпадающие. Отмена поставки не меняет складские остатки; повторное подтверждение исключено.
