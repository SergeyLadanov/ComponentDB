...
- Создание спецификаций вручную и импортом из `.xlsx`, с количеством компонентов на одно устройство и настраиваемым размером тиража.
- Позиции спецификации могут ссылаться на ID склада или задаваться параметрами. Резисторы, конденсаторы и другие пассивы из отчёта PCB BOM Parser автоматически сопоставляются по типу, номиналу, единице, точности и корпусу, когда совпадение однозначно.
- Расчёт потребности показывает требуемое количество, текущий остаток, дефицит и несопоставленные позиции. Excel можно выгрузить как полный список потребности или только список для дозаказа.
- Когда есть дефицит, из списка дозаказа можно сразу создать редактируемую ожидаемую поставку.
...
Архив со страницы GitHub Releases уже содержит собранный интерфейс: после распаковки Node.js не нужен. Установите Python-зависимости, настройте `config.py` при первом запуске и запустите `python run.py` по инструкции ниже.
...
Версия приложения задаётся в `version.py`. Перед выпуском обновите её и добавьте одноимённый раздел в `CHANGELOG.md`, затем запустите вручную workflow **Release project** на нужной ветке. Workflow проверит проект, создаст тег `v<версия>` и GitHub Release с заметками из changelog и архивом `ComponentDB-<версия>.zip`, содержащим готовую сборку интерфейса.
...
## Формат импорта спецификации

Поддерживаются файлы `.xlsx` с первой строкой заголовков. Обязателен столбец
`Количество на устройство` (также принимаются `Количество`, `Кол-во на устройство`
и `Qty`). Позицию можно связать напрямую столбцом `ID компонента` либо описать
столбцами `Классификация`, `Наименование`, `Значение`, `Ед. изм.`, `Точность`,
`Корпус` и `Производитель`. Пустые необязательные поля при сопоставлении не
учитываются.

Также принимается Excel-отчёт PCB BOM Parser со столбцами `Исходное наименование`,
`Тип элемента`, `Параметры`, `Наимен. произв.`, `Производитель` и `Количество`.
Параметры вида `Значение: 10 кОм`, `Точность: 1 %`, `Корпус: 0603` разбираются
автоматически. Если единственного совпадения на складе нет, строка сохраняется как
несопоставленная и целиком попадает в дозаказ.

Поддерживается и компактный трёхколоночный вариант `#` / исходная строка /
количество, включая файлы с повреждённой кодировкой русских заголовков. Строки
вида `12pF 5% 50V NP0 0603`, `0.1uF 5% 16V X7R 0603` и
`10k 5% 0.063W 0603` распознаются как пассивные элементы. Русские и английские
обозначения единиц совместимы, а номиналы сравниваются с учётом масштаба:
например, `100 nF`, `0,1 мкФ` и `0.1 uF` считаются равными.

Выгрузка всей спецификации и списка на дозаказ содержит пять столбцов: `#`,
`Тип элемента`, `Наименование`, `Описание` и `Количество`. Наименование собирается
через пробел из всех заполненных параметров позиции, кроме описания, а также
артикула и производителя. В полной спецификации указывается требуемое количество,
в дозаказе — недостающее.

Для любой несопоставленной строки в окне спецификации доступна команда «Найти на
складе». Поиск работает по ID, артикулу, номиналу, корпусу, описанию,
производителю и ячейке; выбранная позиция сохраняется после нажатия
