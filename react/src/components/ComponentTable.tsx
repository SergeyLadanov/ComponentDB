...
export default function ComponentTable({ rows, highlight, selectedIds, onSelect, onEdit, filters, onFilter, sort, onSort, loading, disabled }: Props) {
  ...
  return <div className="table-scroll" aria-busy={loading}>
    <table className="table component-table mb-0">
      <caption className="visually-hidden">Список электронных компонентов. Можно выбрать несколько строк для удаления; для изменения или списания выберите одну строку.</caption>
      <thead>
        <tr>
      ...
      <tbody>
        {rows.map(component => <tr key={component.id} className={selectedIds.has(component.id) ? 'selected-row' : ''} onClick={() => { if (!disabled) onSelect(component.id) }} onDoubleClick={() => { if (!disabled) onEdit(component) }}>
          <td className="select-column"><input className="form-check-input" type="checkbox" checked={selectedIds.has(component.id)} disabled={disabled} aria-label={`Выбрать позицию ${component.id}`} onClick={event => event.stopPropagation()} onChange={() => onSelect(component.id)} /></td>
