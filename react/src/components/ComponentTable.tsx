...
export default function ComponentTable({ rows, selectedId, onSelect, onEdit, filters, onFilter, sort, onSort, loading, disabled }: Props) {
      ...
      <tbody>
        {rows.map(component => <tr key={component.id} className={selectedId === component.id ? 'selected-row' : ''} onClick={() => { if (!disabled) onSelect(component.id) }} onDoubleClick={() => { if (!disabled) onEdit(component) }}>
          ...
          {columns.map(column => <td key={column.key} className={`cell-${column.key}`}>
            {column.key === 'cnt' ? <span className={`quantity ${Number(component.cnt) === 0 ? 'quantity-empty' : ''}`}>{component.cnt}</span> : component[column.key] || <span className="empty-value">—</span>}
          </td>)}
        </tr>)}
